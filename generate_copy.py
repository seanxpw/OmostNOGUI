...
if __name__ == '__main__':
    ...
    for group_name, group in groups.items():
        ...
        for index, (combo, simplified_name, count, name_list, count_list) in enumerate(zip(combinations, names, counts, yolo_name_lists, yolo_count_lists)):
            if index < skip_count:
                logging.info(f"Skipping combination {index + 1}: {simplified_name}")
                continue
